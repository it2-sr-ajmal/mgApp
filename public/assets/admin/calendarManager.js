var calendarManager = (function(){
	var _this = this, selDate, operationPerformed = false , blockTableID = '#blockingDetailsTable', dynaSpaceSelectBoxClass = '.blockingTblSpaceSelect';
	_this.utils = {
					...
					$('#blockingDetailsTable > tbody > tr').each(function(i,v){
						if($(this).hasClass('spaceTR') && $(this).attr('data-space')){
							...
							if(spaceID > 0 ){
								...
								fromTime = moment( selDate +' '+sT , "YYYY-MM-DD HH:mm");
								toTime = moment( selDate +' '+eT , "YYYY-MM-DD HH:mm");
								
								if(fromTime.diff(toTime) >= 0){
									returnObj.message = "Invalid block times. Please select a valid time.";
									return returnObj;
								}
								fromTimeArr.push(fromTime.format("YYYY-MM-DD HH:mm"));
								toTimeArr.push(toTime.format("YYYY-MM-DD HH:mm"));
				...
				createTableBasedOnSelectableArea: function(selDate){
					...
					var tmp =  moment( selDate +' '+sTime[0], "YYYY-MM-DD HH:mm");
				...
				getOverlappedBookingDetailsTable : function(bookingIDs){
					...
					$(bookingIDs).each(function(i,v){
						...
						$(monthlyUserBookings).each(function(ind,val){ 
							...
							if(val.sb_booking_id == v){
								$tr = $('<tr>');
								var tdHTML = '<td>'+(++slNo)+'</td><td>'+val.sb_booking_id+'</td><td>'+val.user_first_name+' '+val.user_middle_name+' '+val.user_last_name+'</td>';
								tdHTML += '<td>'+val.startTime+' to '+val.endTime+'[ '+ val.sb_duration +' ]' +'</td>';
								tdHTML += '<td>'+val.team_member_name+' - '+((val.sb_room_type)?' Meeting Room ':' Space ') +'</td>';
								tdHTML += '<td>'+val.sb_purpose +'</td>';
								tdHTML += '<td style="text-align:center"><div><input type="checkbox" class="reasonLangSwitch"/> Arabic </div>'+
										  ...
										  ' <button data-id="'+val.sb_booking_id+'" class="cancelBookingBtn">Cancel Booking</button>'+
				...
				getOverlappedEventDetailsTable : function(eventIDs){
					...
					$(eventIDs).each(function(i,v){
						...
						$(monthlyEvents).each(function(ind,val){ 
							if(val.team_id == v){
								$tr = $('<tr>');
								var tdHTML = '<td>'+(++slNo)+'</td><td>'+val.team_member_name+'</td><td>'+val.team_from_date+' '+val.team_to_date+'</td>';
					...
					var slNo =0; 
						...
						$(monthlyBlocks).each(function(ind,val){ 
							if(val.atb_date == dat){
								...
								$tr = $('<tr>');
								var tdHTML = '<td>'+(++slNo)+'</td><td>'+val.team_member_name+'</td><td>'+val.time_from+'</td>';
								tdHTML += '<td>'+val.time_to+'</td>';
								tdHTML += '<td><a class="deleteBlock" data-blockID="'+val.atb_id+'" href="#">Delete</a></td>';
								$tr.append(tdHTML);
								$tbody.append($tr);
	...
	_this.BlockTimeSlots = {
			bindBlockingTableEvents : function(){
				...
				$('#bookingManagerTabs').on('change','.blockTimeSelects',function(e){
					...
					startTime = moment( selDate +' '+sT, "YYYY-MM-DD HH:mm"), endTime = moment( selDate +' '+eT , "YYYY-MM-DD HH:mm"),  
				...
				$('#bookingManagerTabs').on('click','.deleteBlock',function(e){
					...
					var dataToSend = {blockId:blockId , selDate: selDate};
					dataToSend._token = window.Laravel.csrfToken;

<section class="form-group basic_settings multipuploader">
	...
	<div class="row">
		<div class="col-sm-12">
			<div id="old_gallery" class="col-sm-12">
...
<script>
	...
	function delete_gallery_image(id,elem){
		$.ajax({
			url:'<?php echo URL::to(Config::get('app.admin_prefix').'/image-gallery/delete/'); ?>/'+id,
			type:'get',
			async:false,
			data:{},
			dataType:'json',
	...
	function get_Image_gallery(){
		...
		$('#loader').show();
		$.ajax({
			url:'<?php echo URL::to(Config::get('app.admin_prefix').'/image-gallery/get_old_files/?type='.$postType.'&post='.$editID); ?>',
			type:'post',
			async:false,
					...
					$('#old_gallery').append(
						'<div class="col-sm-3">'+
							'<div class="imageHolder">'+
								'<img src="<?php echo asset('storage/app/public/uploads/gallery/small/')?>/'+images[kk].gallery_image_name+'"/>'+
									'<a class="downImg" href="<?php echo URL::to(Config::get('app.admin_prefix').'/image-gallery/download/')?>/'+images[kk].gallery_image_name+'">'+
										'<i class="fa fa-download"></i>'+
									'</a>'+
									'<span class="delGalImg" data-id="'+images[kk].gallery_id+'">x</span>'+
							'</div>'+
	...
	$(document).ready(function(){
		...
		var cusBaseURL = '<?php echo URL::to('/'); ?>';
		...
		var uploader = new plupload.Uploader({
			...
			url : '<?php echo URL::to(Config::get('app.admin_prefix').'/image-gallery/file-upload?type='.$postType.'&post='.$editID)?>',
			...
			init: {
				...
				BeforeUpload:function (up,files){
					...
					$('#loader').show();
					uploader.settings.url = '<?php echo URL::to(Config::get('app.admin_prefix').'/image-gallery/file-upload?type='.$postType.'&post='.$editID)?>';	
				},										
		...
		$('.multipuploader').on('blur','.imageTitle',function(){
			...
			$.ajax({
				url:'<?php echo URL::to(Config::get('app.admin_prefix').'/image-gallery/save-image-title/'); ?>',
				type:'post',
				async:false,
				data:_data,
				dataType:'json',
